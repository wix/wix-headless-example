...
export default function Store() {
    ...
    async function addToCart(product) {
        ...
        if (cart) {
            ...
            if (existingProduct) {
                return addExistingProduct(existingProduct._id, existingProduct.quantity + 1);
            }
        }
        ...
        const {cart: returnedCard} = await myWixClient.currentCart.addToCurrentCart({
    ...
    async function addExistingProduct(lineItemId, quantity) {
        const {cart} = await myWixClient.currentCart.updateCurrentCartLineItemQuantity([{
            _id: lineItemId,
            quantity,
        }]);
                        ...
                        <div className={styles.column}>
                            ...
                            {setShowPremiumModal && (
                                <div>
                                    ...
                                    <p>
                                        This feature is only available to premium users. Please upgrade to access this
                                        feature.
                                    </p>
                                </div>
                            )}
